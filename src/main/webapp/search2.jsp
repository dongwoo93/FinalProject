					...
					<c:forEach var="result" items="${result}" varStatus="status">
						...
						<div class="card" id="card">
							<h4 class="card-title" id="searchTop">
								<img src="루이.jpg" alt="Card image cap" width="30" height="30"
									class="rounded-circle"> <a id="ids"
									href="board.bo?id=${result.id}&cat=1">${result.id}</a>
								<!-- 북마크 -->
								<c:choose>
									<c:when test="${bookmark.containsKey(result.board_seq)}">
										...
										<i value="${result.board_seq}"
											...
											class="far fa-bookmark icon pointer" onclick="unmarkit(this)"></i>
								...
								<c:choose>
									<c:when test="${result3.containsKey(result.board_seq)}">
										...
										<i value="${result.board_seq}"
											style="color: red; cursor: pointer;" id="likecancel"
											class="fas fa-heart" onclick="unlikeit(this)"></i>
										<!-- 좋아요 카운트 -->
										<c:choose>
											<c:when test="${result4[result.board_seq] != null}">
												<p id="p">
													<i value="${result.board_seq}"
														id="count${result.board_seq}"> <c:out
															value="${result4[result.board_seq]}" />명이 좋아합니다
													</i>
												</p>
											</c:when>
											<c:otherwise>
												<p id="p">
													<i value="${result.board_seq}"
														id="count${result.board_seq}"> <c:out
															value="${result4[result.board_seq]}" />
													</i>
												</p>
											</c:otherwise>
										</c:choose>
									...
									<c:otherwise>
										...
										<i value="${result.board_seq}"
											style="color: red; display: none; cursor: pointer;"
